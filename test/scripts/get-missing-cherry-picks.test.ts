...
describe('cases where cherry-picks are missing', () => {
  ...
  it('stable has 2 cherry-picks that are not in beta-opt-in', async () => {
    ...
    when(
      spiedRepos.compareCommitsWithBasehead(
        deepEqual({
          owner: 'ampproject',
          repo: 'amphtml',
          basehead: '2202090001000...725595ac41082b0f9db3103143b04070b0fa7a9e',
        })
      )
    ).thenResolve({
      status: 200,
    ...
    when(
      spiedRepos.compareCommitsWithBasehead(
        deepEqual({
          owner: 'ampproject',
          repo: 'amphtml',
          basehead: '2202090001000...b0fd442d5abc691a3d6eea076eccb83525258b7b',
        })
      )
    ).thenResolve({
      status: 200,
    ...
    const commits = await getMissingCommits(octokit, '2202090001000', releases);
